 ...
 */
export async function fetchDailySummaries({ date, department, page = 1, limit = 20 } = {}) {
  // Prefer a bulk/team endpoint if available in your backend.
  // If not, you'll need to add one — see "Backend Changes Required" in README.
  const params = {
    date,
  ...
  const res = await apiClient.get('/attendance/team-summary', { params })
  return {
    records: (res.data.summaries || []).map(normalizeSummaryRecord),
...
export function computeKPIs(records) {
  ...
  const avgHours = hoursArr.length
    ? (hoursArr.reduce((s, h) => s + h, 0) / hoursArr.length).toFixed(1)
    : '—'
